---
title: "Module 6: Introductory Data Visualization"
...
If you are interested in the information theory underlying the ```ggplot``` plot approach to graphics here are 3 resources I recommend:
...
```{r}
# add the line from the known data generating process
df <- df %>% mutate(y_line = 2 + (beta*x))
plot + geom_point() + geom_line(data=df,aes(x=x,y=y_line),color="blue")
```
...
Grouping variables in boxplots can be useful for things like visualizing changes in seasonal variation over long time horizons:

```{r}
# Daily Data from Kaggle on Weather in Australia 2007-2017.
# https://www.kaggle.com/jsphyg/weather-dataset-rattle-package

aus <- read.csv('data/weatherAUS.csv') %>% 
...
vms <- read.csv('data/vms_fishing_export.csv') %>% 
...
Histograms can compare two distributions.
...
Another way to visualize teh distribution of a variable is with density plots.
...
vms <- vms %>% select(date_time,fishing,speed,long,lat) %>% 
         filter(month(date_time) %in% c(9,10)) %>%
          filter(speed < 50)
...
ggplot(vms, aes(x=speed) ) +
  ...
  geom_label( aes(x=15, y=-0.25, label="Not fishing"), color="#404080") +
...
From [fivethirtyeight.com, *Why we're 3 million russian troll tweets](https://fivethirtyeight.com/features/why-were-sharing-3-million-russian-troll-tweets/)
...
But if your question is *how do I make the dots on my scatter plot blue instead of black?* You should probably expect salty programmers to scream RTFM at you because the manual tells you very clearly what options you can stuff inside ```geom_point(...)``` to change the appearance of your plot.
