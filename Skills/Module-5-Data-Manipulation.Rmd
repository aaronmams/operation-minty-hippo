---
title: "Module-5-Basic-Data-Manipulation"
output:
  html_document:
    df_print: paged
---
...
```{r}
my.list <- list('Ham',c(1:5),letters[1:5])
my.list
...
```{r}
# the python code below will run inside R studio but won't render to html without the 'reticulate' package
library(reticulate)
...
```{python}
# create a small numeric vector (python calls them lists)
x = [1,2,3,4,5]
print(x)
# add 2 to each value in the vector/list
add_2 = [a+2 for a in x]
print(add_2)
```
...
```{r}
ftl_example <- read.csv('data/ftl_example.csv')
ftl_example
```
...
```{r warnings=F}
vessel_info <- read.csv('data/vessel_info.csv')
vessel_info
```
...
```{r}
ftl <- read.csv('data/ftl_example.csv')
vessel_info <- read.csv('data/vessel_info.csv')
...
[The Base R Cheat Sheet](https://rstudio.com/wp-content/uploads/2016/10/r-cheat-sheet-3.pdf). An incredibly handy desk reference on how to do things like:
...
[Data Wrangling with dplyr Cheat Sheet](https://rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf). Another incredibly handy desk reference for folks working in the dplyr environment.

[R for Beginners](https://education.rstudio.com/learn/beginner/). A cool collection of resources put together by the R Studio folks.